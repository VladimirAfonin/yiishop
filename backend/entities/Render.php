...
class Render
{
    ...
    public static function render($result_summary_info, $code, $specsHeadTable = ['title_ru','code','units_en'])
    {
        ...
        foreach($specsHeadTable as $val) {
            $ths = '';
            $ths .= Html::tag('th','url'); // todo exadd +
        ...
        foreach ($result_summary_info as $k => $item) {
            $tds = '';
            if(isset($item['url'])) { $tds .= Html::tag('td',$item['url']); } // todo exadd +
            foreach ($code as $key => $value) {
                $tds .= Html::tag('td', // todo exadd +
                    (isset($item[$key]) && ($key == 'related' || $key == 'family')) // todo exadd +
                        ? (self::getLinksToRelated($item[$key]))
                        : (self::ifWeHaveMultiArr($item, $key)),
//                : ((isset($item[$key]))
...
//                    : ''),
	...
	public static function ifWeHaveMultiArr($item, $key) // todo exadd +
	{
    ...
    public static function getLinksToRelated($arr) // todo exadd +
    {
        $res_arr = [];
