...
function getAllLinks(bool $needXmlQuery) 
{
    $item_links = [];
    if($needXmlQuery) {
...
function getAllPhones($list_all_phones = 'https://www.gsmarena.com/sitemap-phones.xml') 
{
    if(!$xml = simplexml_load_file($list_all_phones)) throw new RuntimeException('cant load file');
    return $xml;
}
function getXmlLinks($needed_link) 
{
...
function needLinkQuery()
{
    ...
    $path = Url::to('@-parsers/gsmarena/files/cron_time.txt');
    $period = 86400;
...
// uncomment for a some items
/*
$item_links = array_filter($item_links, function($v,$k){
  return $k%30===10;
...
foreach($item_links as $k => $url ) {
    ...
    if($name) {
        ...
        $result_data['url'] = Html::a($url, Url::to($url, true));
    }
    ...
    // get hits
    $hits = $htmlDom->query('//li[@class="light pattern help help-popularity"]/span/text()')->item(0)->nodeValue ?? null;
   if($hits) {
    ...
    $ram_size_value = $htmlDom->query('//span[@data-spec="ramsize-hl"]/following::span[1]/text()')->item(0)->nodeValue ?? null; 
    if($ram_size) {
...
///////////// new logic /////
        ...
        switch($some_table) {
                    ...
                    if(isset($speed_info[1]) && !empty($speed_info[1]) && strpos($speed_info[1], 'LTE') != false) {
                        ...
                        if(!empty($speed_lte))  { 
                            ...
                            if(isset($out_speed_lte[0]) && !empty($out_speed_lte[0])) {
                                $result_data['p4zld9l'] = str_ireplace(['Mbps'],'',$out_speed_lte[0]); 
                            }
                        }
                    }
                }
            ...
            case 'Body':
                ...
                if($build_td == 'SIM') {
                    ...
                    if($body_material) { 
                        ...
                        if(isset($out_body[1]) && !empty($out_body[1])) { $result_data['zwkph17b'] = strtolower(trim($out_body[1])); }
                        // get back cover
            ...
            case 'Display':
                ...
                if($display_features_td && $display_features_td != null) {
                    ...
                    if($display_specs) {
                        ...
                        $result_data['ywkph18a'] = isValueExists($display_specs, 'DCI-P3');
                    }
                }
                break;
            case 'Platform':
            ...
            case 'Camera':
                ...
                if($features_td && $features_td == 'Features') {
                  ...
                  $cam_features = $htmlDom->query('//table['.$i.']/tr')->item(1)->nodeValue ?? null;
                    if($cam_features) {
                        // get panorama
                        $result_data['gn4gn6xz'] = isValueExists($cam_features, 'anorama');
                    }
                }
                ...
                if($secondary_td && $secondary_td == 'Secondary') {
                    ...
                    if(isset($cam_secondary_info[0])) {
                        ...
                        if(isset($out_sec_cam[1]) && !empty($out_sec_cam[1])) {
                            $result_data['06wzu4yz'] = $out_sec_cam[1].'MP'; 
                        }
                    }
                }
            ...
            case 'Sound':
                ...
                if($loudspeaker_td && $loudspeaker_td == 'Loudspeaker ') {
                    ...
                    // get THX sound
                    $result_data['yh7xh42'] = isValueExists($sound_loudspeaker, 'THX');
                    // get dual loudspeaker
                    $result_data['yq2jcrl2'] = isCodecExists($sound_loudspeaker, 'dual');
                }
                ...
                if($sound_features_td && $sound_features_td != null) {
                    // get sound specs
                    $sound_specs = $htmlDom->query('//table['.$i.']/tr')->item(3)->nodeValue ?? null;
                    if($sound_specs) {
                        // get hi-fi audio
                        ...
                        // get Dirac HD sound
                        $result_data['yh7xh43'] = isValueExists($sound_specs, 'Dirac HD sound'); 
                    }
                }
                ...
                for($k = 0; $k <= 7;$k++) {
                    ...
                    switch($coms_value_td) {
                        ...
                        case 'Bluetooth':
                            ...
                            // get a2dp
                            $result_data['yh7xh59'] = isValueExists($comms_bluetooth,'A2DP'); 
                            // get LE (low energy)
                            $result_data['8q7wrluz'] = isValueExists($comms_bluetooth,'LE');
                        ...
                        case 'GPS':
                            ...
                            $result_data['x1xgsbl'] = isValueExists($comms_gps, 'A-GPS');
                            break;
                        case 'NFC':
                            // get comms nfc
                        ...
                        case 'Radio':
                           // get comms radio
                            $comms_radio = $htmlDom->query('//table['.$i.']/tr/td[2]')->item($k)->nodeValue;
                            $result_data['tix99ot'] = getAnswerNo($comms_radio);
                            // get RDS
                            if(stripos(trim($comms_radio),'RDS') !== false) $result_data['p4zld1l7'] = getAnswerNo($comms_radio); 
                            break;
                        case 'USB':
                            // get comms usb
                ...
                for($k = 0; $k <= 5; $k++) {
                    ...
                    switch(true) {
                        case ($features_value_td == 'Sensors'):
                            ...
                            $feat_sensors = $htmlDom->query('//table['.$i.']/tr')->item($k)->nodeValue ?? null;
                            if($feat_sensors) {
                            ...
                            $result_data['h88pkmd1'] = isValueExists($feat_sensors, 'oximity'); 
                            }
                            break;
                        case ($features_value_td == 'Messaging'):
                            ...
                            if($feat_messaging) {
                                $result_data['8q7wrlu5'] = isValueExists($feat_messaging, 'IM');
                            }
                            break;
                        ...
                        case (strlen($features_value_td) == 2):
                            ...
                            $result_data['8j6be1ko'] = isCodecExists($feat_browser, 'DivX');
                            ...
                            $result_data['f3n8nq17'] = isCodecExists($feat_browser, 'ASF'); 
                            $result_data['f3n8nq18'] = isCodecExists($feat_browser, 'FLV'); 
                            $result_data['f3n8nq19'] = isCodecExists($feat_browser, 'M4V'); 
                            $result_data['f3n8nq20'] = isCodecExists($feat_browser, 'WEBM');
                            $result_data['f3n8nq21'] = isCodecExists($feat_browser, '3G2'); 
                            $result_data['f3n8nq22'] = isCodecExists($feat_browser, '3GP'); 
                            $result_data['f3n8nqp5'] = isCodecExists($feat_browser, 'AWB'); 
                            $result_data['f3n8nqp6'] = isCodecExists($feat_browser, 'DFF'); 
                            $result_data['f3n8nqp7'] = isCodecExists($feat_browser, 'IMY'); 
                            $result_data['f3n8nqp8'] = isCodecExists($feat_browser, 'RTX'); 
                            $result_data['f3n8nqp9'] = isCodecExists($feat_browser, 'OGA'); 
                            $result_data['f3n8nq11'] = isCodecExists($feat_browser, 'OTA'); 
                            $result_data['f3n8nq10'] = isCodecExists($feat_browser, 'MXMF');
                            $result_data['f3n8nq13'] = isCodecExists($feat_browser, 'AMR'); 
                            $result_data['f3n8nq14'] = isCodecExists($feat_browser, 'APE'); 
                            $result_data['f3n8nq15'] = isCodecExists($feat_browser, 'DSF'); 
                            $result_data['f3n8nq16'] = isCodecExists($feat_browser, 'OGG'); 
                            $result_data['f3n8nq23'] = isCodecExists($feat_browser, 'PCM');
                            
                            // get document viewer 
                            $result_data['yq2jcrl9'] = isValueExists($feat_browser, 'ocument viewer');
                            
                            // get document editor 
                            $result_data['yq2jcr10'] = isValueExists($feat_browser, 'ocument editor');
                    ...
                    switch($value_td) {
                        ...
                        case 'SAR EU';
                            ...
                            if($sar_eu) {
                                ...
                                $sar_info = array_diff($sar_info, array(''));
                                if(isset($sar_info[0]))  $result_data['psbzu2e9'] = str_replace('(head)', '', $sar_info[0]); // todo exadd
                                if(isset($sar_info[1])) $result_data['uuapl9gw'] = str_replace('(body)', '', $sar_info[1]); // todo exadd
                            }
                    }
                }
                break;
    ...
    if(!isset($result_summary_info[$url['drbmx1r']])) {
        $not_gsm_device = $htmlDom->query('//span[@id="non-gsm"]/p/text()')->item(0)->nodeValue ?? null;
        if(!empty($not_gsm_device)) {
            $result_summary_info[$url['drbmx1r']] = 2;
        } else {
            if(isset($result_summary_info[$url['qorav98']])) {
                if($result_summary_info[$url['qorav98']] > 60) $result_summary_info[$url['drbmx1r']] = 1;
            }
            if(isset($result_summary_info[$url['qorav98']])) {
                if($result_summary_info[$url['qorav98']] < 60) $result_summary_info[$url['drbmx1r']] = 2;
            }
        }
    }
    // save data to cache
...
<?= Render::render($result_summary_info, $code) ?>
