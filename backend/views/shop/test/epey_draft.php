...
use backend\views\shop\test\Clerk;
use backend\entities\Render;
use backend\entities\WebPage;
use backend\views\shop\test\H; // todo changed
use \backend\views\shop\test\Sheet;
use yii\helpers\Json;
use yii\helpers\Url;
use yii\helpers\Html; // todo changed

global $dom;

class Epey
{
	public static $version = 4; // todo exadd +
	public static $translations = [
		'materials' => [
			...
			'Suni'                              => 'Artificial',
			'Silikon'                           => 'Silicone',
			'Titanyum'                          => 'Titanium', // todo exadd +
			'Alaşım'                            => 'Alloy',
			'Çinko'                             => 'Zink',
			'Alaşımlı'                          => 'Alloy',
			'Magnezyum' => 'Magnesium', // todo exadd +
			'Alloylı' => 'Alloy',
		],
		'features' => [
			...
			'Yüksek'                                      => 'High',
			'Baş aşağı Kullanabilme'                      => 'Upside down', // todo exadd +
			'Hoparlör'                                    => 'Speaker',
...
//		var_dump($data);
		...
		return $result = str_ireplace(array_keys(self::$translations[$group]), array_values(self::$translations[$group]), $data); // todo +
	}
	...
	public static function categoryLinks($categoryId)
	{
		...
		do {
			...
			$k = 0;
			do {
				$links = $dom->query('//div[@class="detay cell"]/a/@href')->item($k)->nodeValue ?? null; // todo exadd +
				if($links) $urls[] = $links;
				$k++;
			} while($links);

			$i++;
		} while(strlen($html) >= 9500);
	...
	public static function getDataFromApiWithCategory($endpoint, $params = [], $category_id = false)
	{
		...
		$ch = curl_init($url);
		curl_setopt($ch, CURLOPT_HEADER, 0); // todo exadd +
		if($category_id) {
...
//		curl_setopt($ch, CURLOPT_TIMEOUT, 10);
		curl_setopt($ch, CURLOPT_FOLLOWLOCATION, 1); // todo exadd ???


		curl_setopt($ch, CURLOPT_USERAGENT,$_SERVER ['HTTP_USER_AGENT']); // todo exadd ???
	...
	{
//		$path = Url::to("@-parsers/epey/files/map_{$category}.json"); // todo changed
		$path = dirname(__FILE__) . "/../tag/map_{$category}.json"; // todo changed

		if(isset($path)) return Json::decode(file_get_contents($path));
	}

//	public static function getAllLinks(bool $needQuery, $category) // todo exadd +
//	{
//		$item_links = [];
//		if($needQuery) {
//			// get query for all urls phones
//			$item_links_epey = self::getLinksFromJsonFile($category);
//			foreach($item_links_epey as $link) {
//				$item_links[] = self::saveLinks($link);
//			}
//		} else {
...
//	}
	...
	public static function saveLinks($needed_link) // todo exadd + delete
	{
	...
	{
//		$path = Url::to("@-parsers/epey/files/map_{$category}.json"); // todo changed
//		$path = dirname(__FILE__) ."/../tag/map_{$category}.json";
//
//		$period = 60; // 1 day // todo changed
...
//$needLinksQuery = Epey::links(1); // links query
//WebPage::$clean = ['head','script','style'];
$urls = Epey::links(1); // links query // todo exadd +

//var_dump($urls);exit('[exit]');
//$urls = Epey::links(16); // links query // todo exadd +
//$urls = Epey::getAllLinks($needLinksQuery, 1); //PHONE
//$urls = Epey::getAllLinks($needLinksQuery, 16); //WEARABLES
...
$targets = [
//	'https://www.epey.com/akilli-saat/fitbit-versa-se.html',
//	'https://www.epey.com/akilli-saat/fitbit-versa.html',

//	 'https://www.epey.com/akilli-telefonlar/apple-iphone-7.html',
//	'https://www.epey.com/akilli-telefonlar/xiaomi-redmi-5a.html',
...
//		'https://www.epey.com/akilli-telefonlar/xiaomi-mi-mix.html',
//		'https://www.epey.com/akilli-telefonlar/apple-iphone-5s.html',
//	'https://www.epey.com/akilli-telefonlar/huawei-p20-pro.html',
//	'https://www.epey.com/akilli-telefonlar/sony-xperia-z-ultra.html',
//	'https://www.epey.com/akilli-telefonlar/apple-iphone-8-256gb.html',
//'https://www.epey.com/akilli-telefonlar/samsung-galaxy-j7-prime-32gb.html',
...
//	'https://www.epey.com/akilli-telefonlar/xiaomi-redmi-4x-32gb.html',
//	'https://www.epey.com/akilli-telefonlar/xiaomi-redmi-4x-64gb.html',
//	'https://www.epey.com/akilli-telefonlar/xiaomi-redmi-4x.html',//	'https://www.epey.com/akilli-telefonlar/apple-iphone-7.html',
//	'https://www.epey.com/akilli-telefonlar/apple-iphone-8.html',
//	'https://www.epey.com/akilli-telefonlar/apple-iphone-x.html',
//	'https://www.epey.com/akilli-telefonlar/samsung-galaxy-s9.html',
//	'https://www.epey.com/akilli-telefonlar/meizu-m6.html',
//	'https://www.epey.com/akilli-telefonlar/meizu-m6-note.html',
//	'https://www.epey.com/akilli-telefonlar/razer-phone.html',
//	'https://www.epey.com/akilli-telefonlar/sony-xperia-xa1.html',
//	'https://www.epey.com/akilli-telefonlar/oneplus-5t.html',
//	'https://www.epey.com/akilli-telefonlar/xiaomi-redmi-5a.html',
//	'https://www.epey.com/akilli-telefonlar/xiaomi-redmi-4x.html',
//	'https://www.epey.com/akilli-telefonlar/lg-q6.html',
//	'https://www.epey.com/akilli-telefonlar/huawei-nova-2.html',
//	'https://www.epey.com/akilli-telefonlar/google-pixel-2-xl.html',
//	'https://www.epey.com/akilli-telefonlar/huawei-mate-10.html',
//
//	'https://www.epey.com/akilli-telefonlar/samsung-galaxy-j2-2018.html',
//	'https://www.epey.com/akilli-telefonlar/samsung-galaxy-j2-2016.html',
//	'https://www.epey.com/akilli-telefonlar/samsung-galaxy-j2-2017.html',
//	'https://www.epey.com/akilli-telefonlar/samsung-galaxy-j2.html',


//	'https://www.epey.com/akilli-saat/huawei-honor-band-3.html',
//	'https://www.epey.com/akilli-saat/xiaomi-amazfit-cor.html',
//	'https://www.epey.com/akilli-saat/xiaomi-amazfit-bip.html',
//	'https://www.epey.com/akilli-saat/amazfit-pace.html',
//	'https://www.epey.com/akilli-saat/samsung-gear-s3-classic.html',
//	'https://www.epey.com/akilli-saat/fitbit-ionic.html',
//	'https://www.epey.com/akilli-saat/huawei-watch.html',
//	'https://www.epey.com/akilli-saat/huawei-watch-2.html',
//	'https://www.epey.com/akilli-saat/amazfit-arc.html',
//	'https://www.epey.com/akilli-saat/fitbit-charge-2.html',
//	'https://www.epey.com/akilli-saat/xiaomi-mi-band-3.html',
//	'https://www.epey.com/akilli-saat/xiaomi-mi-band-2.html',
//	'https://www.epey.com/akilli-saat/xiaomi-mi-band-1s.html',
//	'https://www.epey.com/akilli-telefonlar/samsung-galaxy-note-5.html',
//	'https://www.epey.com/akilli-telefonlar/samsung-galaxy-note-5-dual-sim.html',
//	'https://www.epey.com/akilli-telefonlar/samsung-galaxy-note-5-64gb.html',

//	'https://www.epey.com/akilli-telefonlar/samsung-galaxy-s9-plus.html',
//	'https://www.epey.com/akilli-telefonlar/samsung-galaxy-s9-plus-128gb-duos.html',
//	'https://www.epey.com/akilli-telefonlar/samsung-galaxy-s9-plus-duos.html',
//	'https://www.epey.com/akilli-telefonlar/samsung-galaxy-s9-plus-256gb.html',
//	'https://www.epey.com/akilli-telefonlar/samsung-galaxy-s9-plus.html',
//	'https://www.gsmarena.com/samsung_galaxy_s9+-8967.php',
//	'https://www.epey.com/akilli-telefonlar/apple-iphone-5s.html',
//'https://www.epey.com/akilli-saat/apple-watch-series-2-38-mm-uzay-siyahi-paslanmaz-celik-kasa-ve-siyah-spor-kordon.html',
	'https://www.epey.com/akilli-telefonlar/cat-s41.html',
	'https://www.epey.com/akilli-telefonlar/turk-telekom-honor-7.html',
	'https://www.epey.com/akilli-telefonlar/turk-telekom-stylus-3.html',
	'https://www.epey.com/akilli-telefonlar/lg-optimus-g-pro-32gb.html',
	'https://www.epey.com/akilli-telefonlar/cat-s60.html',
	'https://www.epey.com/akilli-telefonlar/lg-g-flex.html',
	'https://www.epey.com/akilli-telefonlar/lg-optimus-g-pro.html',

];

//$urls = array_filter($urls, function($v,$k){return $k%2===0;},ARRAY_FILTER_USE_BOTH);

//var_dump($urls);
//exit(); // todo

$items = [];
$clerk = new Clerk('@backend/views/shop/test/clerk.txt', ['total' => count($urls)]);
foreach($urls as $k => $url) {
	$clerk->tick(['url' => $url]);
	// target urls
//	if(array_search($url, $targets) === false) {continue;}

	// get data from cache
	$path_hash = hash('sha256', $url);
	/** @var WebPage $obj */
	$webpage = WebPage::find()->filterWhere(['path_hash' => $path_hash, 'format' => 'json'])->one();
	if($webpage !== null && !empty($webpage->desc) && ($webpage->version == Epey::$version)) { // todo exadd +
	...
	$options = [];
//	$html = Epey::getDataFromApiWithCategory($url);
	...
	if($item_info) {
		...
		$options['w81a9u0'] = str_replace(['Türk'], ['Turkish'], $model_info[0]); // brand // todo exadd +
		$model_name = substr($item_info, strlen($model_info[0]));
//		$model_name = preg_replace('/\(+\d+\)+|2\d{3}/ui', "", trim($model_name)); // old value: todo exadd +
//		$model_name = preg_replace('/\(+\d+\s+GB\)+/ui', "", trim($model_name)); // todo exadd +
...
//		exit();
		...
		$options['ywkph222'] = Epey::translate('words', $options['w81a9u0'] . ' ' . $model_name);
		$model_value = Epey::query('//div[@class="baslik"]/h1/a/span[@class="aile"]/text()');
...
//		$options['model_name'] = Epey::translate('words', $model_name);
...
//		$main_products = array_merge(array_values($options['family']), (array)$url);
...
//		$options['main_product'] = $main_product;
//		if($url == $main_product) {
//			$options['is_main_prod'] = '1';
////			exit('1');
//		} else {
//			$options['is_main_prod'] = '0';
////			exit('0');
//		}
//	} else {
//		$options['main_product'] = $url;
//		$options['is_main_prod'] = '1';
//	}
...
//	exit('done');
	...
	$options['photos'] = [];
	$image = Epey::query('//ul[@class="galerik"]/li/a/img/@src');
	if($image) {
		...
		if(isset($out_url[0]) && !empty($out_url[0])) {
			...
			for($i = 1; $i <= 15; $i++) {
				...
				$picture_png = Epey::getImage($image_url);
				if($picture_png) {
					$options['photos'][] = $picture_png;
				}
			}
			for($i = 1; $i <= 25; $i++) {
				$image_url = $main_url . $fin_url . '-' . $i . '.jpg';
				$picture_jpg = Epey::getImage($image_url);
				if($picture_jpg) {
					$options['photos'][] = $picture_jpg;
				}
			}
		}
	}
	...
	if(strpos($url, 'akilli-saat') === false) {
		...
		for($z = 0; $z <= 6; $z++) {
			...
			if($family_link && $family_title) $options['t40i1m6'][trim(str_ireplace(['Tek Hat', 'Çift Hat', 'Cep Telefonu'], ['one line', 'double line', ''], $family_title))] = trim($family_link); // todo exadd +
		}
		...
		for($b = 0; $b <= 12; $b++) {
			...
			if($related_link && $related_title) $options['t5x1lef'][trim(str_ireplace(['Tek Hat', 'Çift Hat'], ['One Line', 'Double Line'], $related_title))] = trim($related_link); // todo exadd +
		}
		...
		if($display_touch) { // todo exadd +
//			$arr_display = ['Kapasitif Ekran' => 'Capacitive Screen'];
			$options['pcao0re'] = Epey::translate('words', $display_touch); // str_replace(array_keys($arr_display),array_values($arr_display),trim($display_touch));  // touch_type
		}
		...
		if($fast_charging) {
			...
			if($version_info == '+') {
				if(preg_match('/\d+\.*\d*/mui', $version_info, $out_arr_version_charge)) $options['27s8wl4'] = trim($out_arr_version_charge[0]); // todo exadd +
			}
		}
		...
		if($chipset) {
			$chipset_info = Epey::translate('words', $chipset); //trim($chipset); } // todo exadd +
		...
		if($gpu_info) {
			$options['4kzmswo'] = trim(str_replace(['????'],[''], $gpu_info)); // todo exadd +
		}
		...
		if($internal_storage) {
			$options['c8xo6x6'] = trim($internal_storage);
		}
		...
		if($camera_3_res) {
			$options['06wzu1yz'] = str_replace('MP', '', $camera_3_res); // todo exadd +
		}
		...
		for($i = 0; $i <= 5; $i++) {
			...
			if($camera_3_feat) {
				$options['06wz11yz'][] = str_replace(['Optik','Derinlik','Algılama','Sensör'], ['Optic','Deep','Detection','Sensor'], trim($camera_3_feat)); // todo exadd +
			}
		}
		...
		if($sec_aux_proc) {
			$options['y5xo6x8'] = Epey::translate('words', $sec_aux_proc);// str_replace(['Hareketk','Haraket','İşlemcisi','Sensör'],['Motion','Moving','Processor','Sensor'],trim($sec_aux_proc)); // todo exadd +
		}
		...
		if($colors) {
			...
			$color_info = str_replace(['ZirocÃ±an', 'Â'], [''], $color_info); // todo exadd +
			$color_info = preg_replace('/\s/ui', ".", $color_info);
			$options['ywkph10b'] = explode('...', $color_info);
		}

//		H::print_r($options);exit();
		// get cover materials
		$cover_materials_info = Epey::query('//strong[@class="ozellik1320"]/following::span[1]');
//		$cover_materials_info = $dom->query('//strong[@class="ozellik1320"]/following::span[1]')->item(0)->nodeValue ?? null;
//		print_r($cover_materials_info);exit();
		if(trim($cover_materials_info)) { // todo exadd +
//			var_dump($cover_materials_info);var_dump('---in'); // exit();
			$options['3bjbzry'] = Epey::translate('materials', $cover_materials_info);
//			exit();
//			var_dump($cover_materials);echo '<br>';
//			var_dump($options['3bjbzry']);exit();
		}


		// get frame materials
		$frame_materials = Epey::query('//strong[@class="ozellik1321"]/following::span[1]');
		if(trim($frame_materials)) {
			$options['3bjbzra'] = str_ireplace([' )', '+'], [')', ';'], Epey::translate('materials', trim($frame_materials)));
		}
		...
		if($water_resistance_level) {
			...
			$options['cxeplx1'] = str_ireplace(array_keys($arr_wt_resist), array_values($arr_wt_resist), $water_resistance_level); // todo exadd +
		}

		// get dust resistance // todo exadd +
		$dust_resistance = Epey::query('//strong[@class="ozellik330 tip"]/following::span[1]');
		if($dust_resistance) {
			$options['cxeplx2'] = Epey::answer(trim($dust_resistance));
		}

		// get resistance dust level // todo exadd +
		...
		if($user_interface) {
			$options['a1xo6x6'] = Epey::translate('words', $user_interface); //str_ireplace('Saf','Pure',$user_interface); // todo exadd +
		}
		...
		if($ram_channels) {
			$options['v3xo6x6'] = str_replace(['Çift', 'Tek', 'Dört', 'Kanal'], ['Dual', 'Single', 'Four', 'Channel'], $ram_channels); // todo exadd +
		}
		...
		$options['awkph141'] = [];
		$a = 0;
		$all_services_parent_node = $dom->query('//strong[@class="ozellik217 tip"]/following::span[1]')->item(0)->childNodes ?? null; // todo exadd +
		...
		if($usb_info) {
			$options['q8o92fk'] = Epey::translate('words', trim($usb_info)); // todo exadd +
		...
		// get display
//		$display_exist = Epey::query('//strong[@class="ozellik1246"]/following::span[@class="cell cs1"]/span/a/text()'); // screen_size
//		if($display_exist) {
//			$options['aakph191'] = Epey::answer($display_exist); // todo exadd +
//		}
		...
		if($display_colors) {
			$options['8vzzca7'] = str_replace(['Renkli'], ['Color'], explode(' ', $display_colors)[0]); // number of colors
		}
		...
		// charging format // todo exadd + del
//			$charg_format = Epey::query('//strong[@class="ozellik1161"]/following::span[1]'); // battery_capacity
//			if($charg_format) $options['charg_type'] = trim(str_replace(['Şarj İstasyonu'], ['Charging Station'], $charg_format));
		...
		if($required_apps) {
			$options['ywkph13b'] = trim(str_replace(['Gerekli Uygulama','(Android)','Vestel Akıllı Bileklik'], ['','Vestel Smart Wristband'], $required_apps)); // todo exadd +
		}

		// get required apps ios
		$required_apps_ios = Epey::query('//strong[@class="ozellik1230"]/following::span[1]');
		if($required_apps_ios)  {
			$options['awkph14b'] = trim(str_replace(['Gerekli Uygulama','(iOS)'], [''], $required_apps_ios)); // todo exadd +
		}
		...
		if($width) {
			$options['65ihv16'] = str_replace('mm', '', trim($width)); // todo exadd +
		}
...
//		 dust resistance // todo exadd +
//		$dust_resistance = Epey::query('//strong[@class="ozellik1150"]/following::span[1]/span/a/text()');
//		if($dust_resistance) { $options['cxeplx2'] = Epey::answer($dust_resistance);}

		// dust resistance properties
		$dust_resistance_prop = Epey::query('//strong[@class="ozellik1152"]/following::span[1]/span/text()');
		if($dust_resistance_prop) { // todo exadd +
			$options['cxeplx2'] = trim($dust_resistance_prop);
		} else {
			$options['cxeplx2'] = '-';
		}

		// water resistance // todo exadd +
		...
		if($services_apps) {
			...
			$options['zrxr18ut'] = Epey::check($services_apps, 'Kürek Çekme'); // rowing
		...
		if($wifi) $options['2pinrcv'] = str_replace(['Yazılım güncellemesi gerektirebilir','Konum Bilgisi için'], ['Software update may require','For location information'], trim($wifi)); // todo exadd +
...
//	foreach($options as $index => $value) { // todo exadd delete +
//		if(is_string($value) && empty($value)) unset($options[$index]);
//	}

//	var_dump($options['is_main_prod']);
//	exit(); // todo



	$options = array_filter($options, function ($v, $k) {
		return !empty($v); // todo exadd +
		// todo exadd +
	}, ARRAY_FILTER_USE_BOTH);


	// get main product // todo exadd +
	if(isset($options['t40i1m6'])) { // todo exadd +
		$main_products = array_merge(array_values($options['t40i1m6']), (array)$url); // todo exadd +
		usort($main_products, function ($item1, $item2) {
			if(strlen($item1) == strlen($item2)) {
				return 0;
			}
		...
		$main_product = $main_products[0];
		$options['g55z4fn'] = $main_product; // todo exadd +
		if($url == $main_product) {
			$options['rt6iqm1'] = 1; // todo exadd +
//			exit('1');
		} else {
			$options['rt6iqm1'] = 0; // todo exadd +
//			exit('0');
		}
	} else {
		$options['g55z4fn'] = $url; // todo exadd +
		$options['rt6iqm1'] = 1; // todo exadd +
	}
...
//		$newObj->source = 'epey';
//		$newObj->url = $url;
//		$newObj->nid = Epey::$version;
//		$newObj->desc = Json::encode($items[$url]);
//		$newObj->save();
//	}

//	$newObj = WebPage::findOne(['path_hash' => $path_hash]);
	if($webpage != null) {
		$webpage->version = Epey::$version; // todo exadd +
...
//		$newObj->source = 'epey';
//		$newObj->url = $url;
//		$newObj->version = Epey::$version; // todo exadd +
//		$newObj->desc = Json::encode($items[$url]);
//		$newObj->save();
	}
...
$codes = Sheet::rf('@backend/views/shop/test/specs.csv', ['indexFrom' => 'code']); // todo changed
H::print_r($items);
echo(Render::render($items, $codes));
//H::print_r($items);
/*$services = [];
foreach ($items as $options){:
	if(isset($options['awkph141'])){
		foreach ($options['awkph141'] as $service){
...
foreach ($webpages as $webpage){
	...
	$device = trim($row['w81a9u0']).' '.$row['33fksng'];
	$row['url'] = '';
	$row['drbmx1r'] =[];
	$data[$webpage->url] = $row;
}
//$specs = Sheet::rf('@config/product/specs.csv', ['indexFrom' => 'code']);
$specs =  Sheet::rf('@backend/views/shop/test/specs.csv', ['indexFrom' => 'code']);
...
<table class="table table-bordered table-micro">
	<? foreach ($specs as $code=>$value):?>
		<tr>
			<td><?=$specs[$code]['category_ru']??'' ?></td>
			<td><?=$specs[$code]['group_ru']??'' ?></td>
			<td><?=$code?></td>
