...
class Pleer
{
	...
	public static $clean = ['script','head'];
...
//	exit(); // todo
	...
	// get price
//	$price = $dom->query('//table[@class="product_price"]/tbody/tr/td[@class="id3 "]/span[@class="inlineb"]')->item(0)->nodeValue; // ?? null;
//	$price = $dom->query('//div[@class="product_price_wrap"]')->item(0)->nodeValue; // ?? null;
	$t = $dom->query('//*')->item(0)->nodeValue ?? null;

//	div[contains(@class, 'n-filter-picker__img radiobox__box')]

	var_dump(Pleer::getValue($t));
	exit('[:exit:]'); // todo

//	$dom->query('//table[@id="proxylisttable"]/tbody/tr/td')->item(4)->nodeValue ?? null;
