...
class GSMarena
{
	...
	{
		$links = [];
		$path = dirname(__FILE__) . "/../tag/gsmarena/map.json";
		$period = 30; // 1 day
		if(file_exists($path)) {
...
//exit(); // todo
$targets = [
//	'https://www.gsmarena.com/nokia_2652-1290.php',
...
//'https://www.gsmarena.com/xiaomi_redmi_4_(4x)-8608.php',
...
//$urls = array_filter($urls, function ($v, $k) {return $k % 20 === 0;}, ARRAY_FILTER_USE_BOTH);
// result data info
$items = [];
...
foreach($urls as $k => $url) {
	...
	$result_data = [];
//	$html = GSMarena::getDataFromApi($url);
	$html = WebPage::get($url); // todo exadd +
...
//		var_dump($result_data['ywkph222']);exit();
			...
			case 'Body':
				...
				if($dimensions_td == 'Dimensions') {
					...
					if($body_dimensions) {
						...
						if(isset($body_info[0])) $result_data['qorav98'] = str_replace([' mm', 'mm', 'thickness'], [''],trim($body_info[0])); // todo exadd +
						if(isset($body_info[1])) $result_data['65ihv16'] = str_replace([' mm', 'mm', 'thickness'], [''],trim($body_info[1])); // todo exadd +
...
//									$image_info = str_replace('AMOLED or SLCD', 'AMOLED', $display); // todo exadd +
//									if(preg_match('/\d{1,}k?\s+colors/mui', $image_info, $out_display_colors)) {
//										$result_data['8vzzca7'] = str_ireplace('colors','',trim($out_display_colors[0]));
//									} else {
//										$result_data['xxyv5nx'] = $image_info;
//									}
//								} else {
...
//						if($display_colors) {
//							if(!isset($result_data['8vzzca7'])) { // todo exadd +
...
//					}
					if($display_type) {
						if(strpos($display_type, 'onochrome') != false) {
							...
							$display = str_ireplace(['capacitive', 'touchscreen'], '', explode(',', $display_type)[0]); // todo exadd +
							if( ! empty($display)) {
								if( ! preg_match('/\d+K|\d+M/uim', $display)) {
									/////
									...
									if(!isset($out_display_colors[0])) {
										$result_data['xxyv5nx'] = str_replace('(single white color)','',$image_info); // todo exadd +
									} else {
...
//						$display_colors = explode(',', $display_type)[1] ?? ''; // todo exadd +
//						if($display_colors) {
...
//						}
				...
				for($k = 0; $k <= 7; $k++) {
					...
					switch($coms_value_td) {
						...
						case 'Bluetooth':
							...
							// get a2dp
							$result_data['yh7xh59'] = GSMarena::isValueExists($comms_bluetooth, 'A2DP');
							// get LE (low energy)
							$result_data['8q7wrluz'] = GSMarena::isValueExists($comms_bluetooth, 'LE');
							// get aptX // todo exadd +
				...
				for($k = 0; $k <= 5; $k++) {
					...
					switch(true) {
						...
						case (strlen($features_value_td) == 2):
							...
							// get document viewer 
							$result_data['yq2jcrl9'] = GSMarena::check($feat_browser, 'ocument viewer'); // todo exadd +

							// get document editor 
							$result_data['yq2jcr10'] = GSMarena::check($feat_browser, 'ocument editor'); // todo exadd +
	...
	// save data to db // todo exadd + all
	if($webpage != null) {
...
//		$newObj->url = $url;
//		$newObj->version = GSMarena::$version;
